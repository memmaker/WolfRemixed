...
namespace Twengine.SubSystems
{
    ...
    {
        private ComponentMapper<Physics> mVelocityMapper;
        ...
        protected override void ProcessEntities(Dictionary<int, Entity> entities)
        {
            ...
            for (int j = 0; j < entities.Count; j++)
            {
                ...
                if (!mCollisionQuickCheck[entity.CollisionGroup]) continue;

                Collider entityCollider = mColliderMapper.Get(entity);
                Transform entityTransform = mTransformMapper.Get(entity);
                Physics entityPhysics = mVelocityMapper.Get(entity);
                ...
                {
                    if (!mCollisionMatrixNew[entity.CollisionGroup, other.CollisionGroup]) continue;

                    Collider otherCollider = mColliderMapper.Get(other);        // performance hit
                    Transform otherTransform = mTransformMapper.Get(other);
                    Physics otherPhysics = mVelocityMapper.Get(other);
        ...
        private Bag<Entity> GetArray(Dictionary<int, Entity>.ValueCollection values)
        {
            int counter = 0;
